...
export async function runProcess(
  ...
  const body = {
    ...
    pra_positivity_threshold: options.praPositivityThreshold ?? 20.0,
  ...
  if (!response.ok) {
    throw new Error(await readApiError(response, "Could not start image analysis"));
  }
...
export async function fetchProgress(jobId: string): Promise<BackendProgress> {
  ...
  if (!response.ok) {
    throw new Error(
      await readApiError(response, "Could not load image-analysis progress"),
    );
  }
