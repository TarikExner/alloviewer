...
from .config import ClustererConfig, PARCConfig, FlowSOMConfig, HDBSCANConfig
...
class FlowSOMClusterer:
    """
    Adapter around flowsom.models.FlowSOMEstimator.
    """
    ...
    def _get_training_labels(self) -> np.ndarray:
        if self.label_kind == "cluster":
            # if not hasattr(self.model, "cluster_labels"):
            #     raise AttributeError("FlowSOMEstimator has no attribute 'cluster_labels'.")
            return np.asarray(self.model.cluster_labels, dtype=int)

        if self.label_kind == "metacluster":
            # if not hasattr(self.model, "metacluster_labels"):
            #     raise AttributeError("FlowSOMEstimator has no attribute 'metacluster_labels'.")
...
class PARCClusterer(PARC):
    """
    Adapter that makes PARC fit a BaseClusterer-like interface.
    """
