...
def _generate_main_figure(
    ...
    def generate_subfigure_b(
        ...
        # 3x5 grid: row 0 = GT, row 1 = U-Net, row 2 = ImageJ
        ...
        # ---- row 2: image + ImageJ segmentations ----
        ax_img_ij = fig.add_subplot(fig_sgs[2, 0])
        ax_img_ij.imshow(img)  # same RGB image for reference
        ...
        for c in range(4):
            ...
            ax_ij.set_title(f"{titles[c + 1]} (ImageJ)", fontsize=9)
            ax_ij.set_xticks([])
            ax_ij.set_yticks([])
        ...
        ax_img_ij.set_ylabel("ImageJ", fontsize=8)



    
    fig = plt.figure(
        layout="constrained", figsize=(cfg.FIGURE_WIDTH_FULL, cfg.FIGURE_HEIGHT_FULL*0.7)
    )
...
def figure_S9_generation(
    ...
    **kwargs
    ...
    data = pd.read_csv("../scripts/results/testing_val_imageJ_small_inst_seg.csv")
    ...
    ij_rgb, ij_tgts = fetch_image_with_targets(
        h5_path=ij_path,
        index=img_index,
        tile_idx=tile_idx,
        image_key="imgs",
