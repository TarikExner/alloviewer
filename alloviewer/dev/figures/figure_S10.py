...
def _prepare_runtime_data(summary_df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    focus_df["gate_display"] = focus_df["gate"].map(gate_display_map)
    focus_df["algorithm_display"] = focus_df["algorithm"].map(algo_display_map)
    ...
    scatter_df = (
        ...
        .agg(
            ...
            memory_current_mb=("memory_current_mb", "mean"),
            memory_delta_mb=("memory_delta_mb", "mean"),
            memory_peak_mb=("memory_peak_mb", "mean"),
        )
    )
    ...
    best_df["algorithm_display"] = best_df["algorithm"].map(algo_display_map)
...
def _generate_subfigure_a(
    ...
    def _sort_key(algo: str, param_json: str):
        ...
        if algo == "hdbscan":
            ...
            return (method_order.get(params["cluster_selection_method"], 999), params["min_samples"])
        return str(params)
    ...
    panel_a_df["gate_display"] = panel_a_df["gate"].map(gate_display_map).fillna(panel_a_df["gate"])
    ...
    for i, algo in enumerate(algo_order):
        ...
        setting_order_df = (
            ...
            .drop_duplicates()
            .assign(sort_key=lambda d: d["algorithm_params_json"].map(lambda s: _sort_key(algo, s)))
        ...
        pivot_df = (
            ...
            .reindex(index=gate_order, columns=param_order)
        )
        ...
        legend_ax.axis("off")
        handles = [
            mpatches.Patch(facecolor=param_to_color[p], edgecolor="none", label=p)
...
def _generate_subfigure_b(
    ...
    sub_ax.set_ylabel("Mean F1-score\n(Lymphocytes, T-cells, B-cells)", fontsize = cfg.AXIS_LABEL_SIZE)
    sub_ax.set_xscale("log")
    sub_ax.set_title("Accuracy versus runtime across all settings", fontsize=cfg.TITLE_SIZE)
    sub_ax.set_ylim(0.0, 1.02)
    sub_ax.legend(frameon=False, title="Algorithm family", fontsize = cfg.TITLE_SIZE,
...
def _generate_main_figure(
    ...
    _generate_subfigure_b(fig, fig_b, b_coords, "B", scatter_df=scatter_df)
    _generate_subfigure_c(fig, fig_c, c_coords, "C", runtime_breakdown_df=runtime_breakdown_df)
